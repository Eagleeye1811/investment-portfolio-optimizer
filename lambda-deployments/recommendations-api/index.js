...
function generateRecommendations(portfolio, prices, sentiment, metrics) {
  ...
  for (const asset of portfolio.assets) {
    ...
    // 1. CRITICAL: Huge loss with negative sentiment (cut losses immediately)
    if (hasNegativeSentiment && profitLossPercent < -30) {
      action = 'SELL';
      confidence = Math.min(95, negativeScore * 100);
      reasoning.push(`Strong negative sentiment (${(negativeScore * 100).toFixed(0)}%)`);
      reasoning.push(`Currently down ${Math.abs(profitLossPercent).toFixed(1)}% - significant loss`);
      reasoning.push('Exit position immediately to limit further damage');
      if (isOverweighted) {
        reasoning.push(`Position is overconcentrated at ${portfolioWeight.toFixed(1)}% of portfolio`);
      }
    }
    
    // 2. Moderate loss with negative sentiment (cut losses)
    else if (hasNegativeSentiment && profitLossPercent < -10) {
      action = 'SELL';
      confidence = Math.min(90, negativeScore * 100);
      reasoning.push(`Strong negative sentiment (${(negativeScore * 100).toFixed(0)}%)`);
      reasoning.push(`Currently down ${Math.abs(profitLossPercent).toFixed(1)}%`);
      reasoning.push('Cut losses to prevent further decline');
    }
    
    // 3. Negative sentiment with profit (lock in gains before decline)
    else if (hasNegativeSentiment && profitLossPercent > 5) {
      action = 'SELL';
      confidence = Math.min(92, negativeScore * 100);
      reasoning.push(`Strong negative sentiment (${(negativeScore * 100).toFixed(0)}%)`);
      reasoning.push(`Lock in ${profitLossPercent.toFixed(1)}% profit before potential decline`);
    }
    
    // 4. Overconcentrated with good profit (rebalance) - BUT NOT if strong positive sentiment
    else if (isOverweighted && profitLossPercent > 30 && !hasPositiveSentiment) {
      action = 'SELL';
      confidence = 80;
    ...
    // 5. Declining sentiment trend with profit
    else if (hasNegativeSentiment && sentimentTrend === 'declining' && profitLossPercent > 0) {
      action = 'SELL';
      confidence = 75;
      reasoning.push(`Negative sentiment (${(negativeScore * 100).toFixed(0)}%) with declining trend`);
    ...
    // 6. Positive sentiment with temporary dip (average down)
    else if (hasPositiveSentiment && profitLossPercent < -5 && profitLossPercent > -30) {
      action = 'BUY';
    ...
    // 7. Positive sentiment, underweight, improving trend
    else if (hasPositiveSentiment && portfolioWeight < 15 && sentimentTrend === 'improving') {
      action = 'BUY';
      confidence = 85;
      reasoning.push(`Strong positive sentiment (${(positiveScore * 100).toFixed(0)}%)`);
    ...
    // 8. Winner with strong positive sentiment (let it run)
    else if (profitLossPercent > 5 && hasPositiveSentiment) {
      action = 'HOLD';
      confidence = 80;
      ...
      if (isOverweighted) {
        reasoning.push('Monitor position size - consider taking partial profits if weight exceeds 40%');
      }
    }
    
    // 9. Moderate profit with neutral sentiment
    else if (profitLossPercent > 0 && profitLossPercent < 30 && !hasNegativeSentiment) {
      action = 'HOLD';
      confidence = 70;
      reasoning.push(`Currently up ${profitLossPercent.toFixed(1)}%`);
    ...
    // 10. Small profit/loss with neutral sentiment
    else if (Math.abs(profitLossPercent) < 10 && !hasNegativeSentiment && !hasPositiveSentiment) {
      action = 'HOLD';
      confidence = 65;
      reasoning.push(`Position near entry price (${profitLossPercent >= 0 ? '+' : ''}${profitLossPercent.toFixed(1)}%)`);
      reasoning.push('Sentiment neutral - wait for clearer trend');
    }
    
    // 11. Default hold with specific reasoning
    else {
      action = 'HOLD';
      confidence = 60;
      if (profitLossPercent < -5) {
        reasoning.push(`Currently down ${Math.abs(profitLossPercent).toFixed(1)}%`);
        reasoning.push('Monitor position closely for trend changes');
      } else if (profitLossPercent > 0) {
        reasoning.push(`Currently up ${profitLossPercent.toFixed(1)}%`);
        reasoning.push('Maintain current position');
      } else {
        reasoning.push('Position near entry price');
        reasoning.push('Hold and monitor');
      }
    }
